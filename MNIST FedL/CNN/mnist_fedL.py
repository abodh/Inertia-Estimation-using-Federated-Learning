...
if __name__ == '__main__':
    ...
    data_path = "C:\\Users\\abodh\\Box Sync\\Box Sync\\Spring 2020\\inertia project\\matlab files\\area12_comb_nonIID\\manipulated\\"
    ...
    net_glob = Net(n_inp, n_hid1, n_hid2, n_out, 0.5, 0.05, True)
    ...
    for iter in range(args.epochs): # communication rounds
        ...
        weight_ho.append(np.reshape(net_glob.oupt.weight.data.clone().cpu().numpy(), (1, n_hid2 * n_out)))
    ...
    plt.savefig('who.png', dpi=600, bbox_inches='tight')
    plt.show()
    plt.close()

    # plot loss curve
    plt.figure()
    plt.plot(range(len(loss_train)), loss_train, linewidth = 2, label = 'training loss', c='blue' )
    plt.plot(range(len(valid_loss)), valid_loss, linewidth = 2, linestyle = '--',
